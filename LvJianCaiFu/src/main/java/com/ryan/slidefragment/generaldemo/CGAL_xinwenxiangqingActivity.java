...
public class CGAL_xinwenxiangqingActivity extends Activity implements
		OnClickListener {
...
//	BaseApplication app;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_cgal_xinwenxiangqing);
		...
		ids=(String) intent.getSerializableExtra("id");
		System.out.println("is="+ids);
		
//		app = (BaseApplication) getApplication();
//			xinwen_biaoti.setText(app.gettit());
		
			
//		BaseApplication.xinwen_biaoti = null;
//		BaseApplication.xinwen_neirong = null;
		...
		ThreadUtils.newCachedThreadPool().execute(new Runnable() {
			...
			public void run() {
				final String resultFService = HttpClientDao
						.getListHttpClientPost(Constants.SHOUYEXINWENXIANGQING,
								ha);
				...
				if (judger) {
					...
					ThreadUtils.post(new Runnable() {
						...
						public void run() {
							Toast.makeText(CGAL_xinwenxiangqingActivity.this, "网络异常", Toast.LENGTH_LONG).show();
						}
					});
				}
			}
	...
	private void setData(String resultFService) {
		syxwxq = JsonUtils.parser(resultFService,ShouYeXinWenXiangQingBean.class);
