...
import com.ryan.slidefragment.IBtnCallListener;
import com.ryan.slidefragment.Data.Data;
import com.ryan.slidefragment.adapter.Adapter_ListView_cart;
import com.ryan.slidefragment.adapter.Adapter_ListView_cart.onCheckedChanged;
import com.ryan.slidefragment.generaldemo.BabyActivity;
		...
		cb_cart_all.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton arg0, boolean arg1) {
				...
				if (arg1) {
					...
					//判断列表中数是否等于列表的总数，如果等于，那么久需要执行全部取消操作
					if (isChoice_all == Data.arrayList_cart.size()) {
						//如果没有选中了全部，那么就将列表的每一行都选中
						for (int i = 0; i < Data.arrayList_cart.size(); i++) {
							//列表每一行都取消
...
//		btnCallListener = (IBtnCallListener)activity;
	...
	public void transferMsg() {
		...
		System.out.println("由Activity中传来的消息");
	...
	public void getChoiceData(int position, boolean isChoice) {
		...
		//如果选中的状态数量==列表的总数量，那么就将全部设置为选中
			cb_cart_all.setChecked(true);
		}else{
			//如果选中的状态数量！=列表的总数，那么就将全部设置为取消
...
//			Intent intent = new Intent(getActivity(),ActivityChanPinZhanShi.class);
//			startActivity(intent);
//			getActivity().finish();
			break;
		case R.id.tv_cart_buy_or_del://点击结算/删除
			...
			if (tv_cart_buy_Ordel.getText().toString().equals("删除")) {
				...
				//执行结算操作
				Toast.makeText(getActivity(), "暂时还未开发", Toast.LENGTH_SHORT).show();
			}
			break;
		default:
			break;
